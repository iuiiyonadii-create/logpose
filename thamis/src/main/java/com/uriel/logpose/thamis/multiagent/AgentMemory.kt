...
import com.uriel.logpose.data.local.DatabaseProvider
...
class AgentMemory(private val agentName: String) {
    ...
    fun store(task: String, result: String, successful: Boolean = true) {
        ...
        try {
            scope.launch {
                val dao = DatabaseProvider.getDatabase().agentMemoryDao()
                dao.insert(
                    AgentMemoryEntity(
                        agentName = agentName,
    ...
    fun clear() {
        ...
        try {
            scope.launch {
                DatabaseProvider.getDatabase().agentMemoryDao().clearAgent(agentName)
            }
        } catch (e: Exception) {
