...
import com.uriel.logpose.core.data.DriverProfileStore
...
object ActionMapper {
    ...
    private fun limpiarArticulosYConectores(target: String): String {
        var texto = com.uriel.logpose.features.voice.MusicVocabulary.normalize(target).trim()
    ...
    fun ejecutarAccionDirecta(context: Context, jsonString: String) {
        try {
            ...
            LogPoseLogger.d("ActionMapper", "Alineación JSON Exitosa. Acción detectada: $action")

            when (action) {
                "MEDIA_PLAY" -> {
                    val target = json.optString("target", "")
                    if (target.isNotBlank()) {
                        com.uriel.logpose.features.music.engine.SpotifyIntentDispatcher.ejecutarIntentSpotify(context, target)
                    }
                }
                "NAVIGATE" -> {
                    val location = json.optString("location", "")
                    if (location.isNotBlank()) {
                        com.uriel.logpose.features.navigation.engine.GoogleMapsDispatcher.lanzarNavegacion(context, location)
                    }
                }
                "MEDIA_CONTROL" -> {
                    val command = json.optString("command", "")
                    com.uriel.logpose.features.music.engine.HardwareMediaController.executeCommand(context, command)
                }
                "OPEN_APP" -> {
                    val packageName = json.optString("package", "")
                    if (packageName.isNotBlank()) {
                        val launcher = com.uriel.logpose.core.app.AppLauncherImpl(context)
                        launcher.openApp(packageName)
                    }
                }
                "IGNORE" -> {
                    LogPoseLogger.d("ActionMapper", "Filtro UNK/Chaos: El buffer fue clasificado como ruido residual.")
                }
                else -> LogPoseLogger.w("ActionMapper", "Aviso: Se recibió una acción no indexada en el contrato: $action")
            }
        } catch (e: Exception) {
            LogPoseLogger.e("ActionMapper", "CRÍTICO: Quiebre de contrato JSON con Gemini: ${e.message}")
        }
    }
}
